                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            @if($supplier_invoice)
                                     @if ($supplier_invoice->name_plan == 'FREE') 
                                     <h3>Plano Gratuito</h3> 
                                     @else
                                    <h3>{{$supplier_invoice->name_plan}}</h3>
                                     @endif
                                     <p>
                                   
                                     @if ($supplier_invoice->subscription_status == 'active')
                                     {{ trans('supplier.situacao') }}: <b>{{ trans('supplier.ativa') }}</b><br>
                                     @else
                                     {{ trans('supplier.situacao') }}: <b>{{ trans('supplier.inativa') }}</b><br>
                                     @endif
                                     {{ trans('supplier.price') }}: <b>R$ {{number_format($supplier_invoice->valor,2,",",".")}}</b>
                                        <br>
                                        Método de pagamento: <b></b>
                                        <br>
                                        Proximo Vencimento: <b>{{date('d/m/Y', strtotime($supplier_invoice->due_date))}}</b>
                                    </p>

                             @else 

                             <h3>Nenhum plano Contratado</h3> 
                    ...
                    <div class="form-group text-center">
                                        <a href='{{route('supplier.plans.indexplan')}}' class="btn btn-primary mb-3" style="color: #fff; margin-right: 0;">Alterar Plano</a>
                                    </div>
            ...
            <div class="card shadow">
                ...
                <div class="table-responsive">
                    ...
                    <table class="table align-items-center table-flush">
                        ...
                        <tbody>
                            @forelse($supplier_invoice_all as $invoice)
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($invoice->payment == 'paid' ) 
                                    <a href="{{ route('supplier.plans.pay', $invoice->id) }}" class="btn btn-primary" role='button'>Detalhe</a>
                                  
                                    @else 
                                    <a href="{{ route('supplier.plans.pay', $invoice->id) }}" class="btn btn-success" role='button'>Pagar</a>
